...
import com.hero.rssreader.database.utils.BeanUtils;
...
public class DhDB {
	...
	public void init(Context context,String dbname, int dbversion) {
		this.db = new SqliteDbHelper(context, dbname, dbversion)
				.getWritableDatabase();
	};
	 ...
	 * @param dbversion
	 */
	public void initInSD(Context context,String sdcardPath, String dbname, int dbversion) {
	...
	public void save(Object obj) {
		...
		String pk = entity.pk;
		Object pkvalue = BeanUtils.getProperty(obj, pk);
		// 存在id就更新否则插入
		if (pkvalue != null && !pkvalue.toString().equals("0")) {
			...
			if(pkv>0){
				BeanUtils.setProperty(obj, pk,pkv);
			}
		}
	...
	private <T> T cursorToBean(Cursor cursor, Class<T> clazz) {
		...
		for (Iterator<String> iterator = keys.iterator(); iterator.hasNext();) {
			...
			if (field.getType().equals(Integer.class)
				...
				BeanUtils.setProperty(obj, key, cursor.getInt(cursor
	...
	private static String getCreatTableSQL(Class<?> clazz) {
		...
		for (Iterator<String> iterator = keys.iterator(); iterator.hasNext();) {
			...
			Class<?> dataType = BeanUtils.getDeclaredField(clazz, key).getType();
